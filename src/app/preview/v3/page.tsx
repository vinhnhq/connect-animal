    ...
    <div className="min-h-screen bg-white text-black dark:bg-black dark:text-white">
      <div className="mx-auto max-w-4xl px-4 py-8">
        <header className="border-y-[3px] border-black py-6 dark:border-white">
          <h1 className="text-6xl font-black uppercase leading-[0.85] tracking-tighter">
            Connect
            <br />
        ...
        <main className="mt-8">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button
              type="button"
              className="border-[3px] border-black bg-white px-5 py-2 text-base font-bold uppercase tracking-wide hover:bg-black hover:text-white dark:border-white dark:bg-black dark:hover:bg-white dark:hover:text-black"
            >
              Hint
            </button>
            <button
            ...
            <button
              type="button"
              className="border-[3px] border-black px-5 py-2 text-base font-bold uppercase tracking-wide text-white dark:border-white"
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex flex-col">
      <span
        className="px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-white"
        style={{ backgroundColor: accent ? ACCENT : "black" }}
      >
        {label}
      </span>
      <span className="px-3 py-3 font-mono text-2xl font-bold tabular-nums">{value}</span>
    </div>
  );
}
...
function BlogTeaser() {
  return (
    <section className="mt-16 border-t-[3px] border-black pt-6 dark:border-white">
      <span
        className="inline-block px-2 py-0.5 text-xs font-bold uppercase tracking-[0.2em] text-white"
        style={{ backgroundColor: ACCENT }}
      >
        Posts
      </span>
      ...
      <time className="mt-2 block font-mono text-xs uppercase tracking-[0.18em]">
        {BLOG_PREVIEW.date}
      </time>
