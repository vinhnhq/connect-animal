...
// Connect Animal design system — pieces of V3 (the locked direction).
// Sticky TOC on the left at lg+; horizontal chip strip on mobile.

const COLS = 8;
const ROWS = 6;
type SampleCell = { animal: string | null; col: number; row: number };
const SAMPLE_BOARD: SampleCell[] = (() => {
  const RAW = [
    "🐱.🐶..🐭🐰.",
    ".🦊🐻.🐼🐯🐸.",
    "🐱🐶..🐭🐰..",
  ...
  for (let row = 0; row < ROWS; row++) {
    ...
    for (let col = 0; col < COLS; col++) {
      const ch = chars[col];
      out.push({ col, row, animal: !ch || ch === "." ? null : ch });
    }
  }
  return out;
})();
